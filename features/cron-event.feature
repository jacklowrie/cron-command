Feature: Manage WP Cron events
  ...
  Scenario: --due-now with supplied events should only run those
    ...
    When I run `wp cron event unschedule wp_cli_test_event_2`
    Then STDOUT should contain:
      """
      Success: Unscheduled event with hook 'wp_cli_test_event_2'
      """

    When I run `wp cron event schedule wp_cli_test_event_2`
    Then STDOUT should contain:
      """
      Success: Unscheduled events with hook 'wp_cli_test_event_2'
      """
